...
public class Shop {
	...
	// 판매현황
	ArrayList<EveryThing> sell = new ArrayList<>();
	...
	private void saleStatus() {
		...
		for (int i = 0; i < sell.size(); i++) {
			System.out.printf("카테고리 : %s, 상품명 : %s, 가격 : %d , 남은재고 : %d \n", sell.get(i).getCategory(), sell.get(i).getProduct(), sell.get(i).getPrice(), sell.get(i).getStock());
			total += sell.get(i).getPrice();
		}
		System.out.println("=========================================================================================");
	...
	private void changeProduct(EveryThing[] everything) {
		...
		System.out.println("1. 가격, 2. 재고, 0. 이전페이지로 돌아가기");
		...
		if (number == 0) {
			System.out.println("가격 및 재고수정 페이지로 돌아갑니다.");
			change();
		}
		 if (number == 1) {
	...
	private void cart() {
		...
		System.out.println("현재 확률 " + luckylevel(totals));
	...
	private void Purchase(int totals) {
		System.out.println(" =================================");
		...
		switch (number) {
			...
			default: System.out.println("잘못된 입력하셨습니다. 다시 입력해주세요.\n"); 
						...
						Purchase(totals); break;
			
			}	
	}
	// 구매 금액에 따른 행운의 번호 추첨
	private void lucky(int totals) {	 
		 int select = 0;
		 int luckyNumber = 0;
		 int count = 0;
		 boolean isRunning = true;
		if (luckylevel(totals).equals("1%")) {
			count = 100;
			luckyNumber = random.nextInt(count)+1;  // 100까지 랜덤
			System.out.println("행운의 번호 1 ~ 100 사이의 숫자를 입력해주세요.");
		} else if (luckylevel(totals).equals("5%")) {
			count = 50;
			luckyNumber = random.nextInt(count)+1;  // 50까지 랜덤
			System.out.println("행운의 번호 1 ~ 50 사이의 숫자를 입력해주세요.");
		} else if (luckylevel(totals).equals("10%")) {
			count = 10;
			luckyNumber = random.nextInt(count)+1;	// 10까지 랜덤
	...
	private String luckylevel(int totals) {
		String result = "";
		if (totals <= 500000) {
			result = "1%";
		} else if (totals <= 2000000) {
			result = "5%";
		} else {
			result = "10%";
		}
